...
def parse_args():
    ...
    if config_path.endswith('.py'):
        ...
        raise IOError('Only .py file are supported now!')
...
def build_scheduler(optimizer,config):
    ...
    if sch_type == 'LambdaLR':
        ...
        # 等间隔调整学习率，调整倍数为gamma倍，调整间隔为step_size,间隔单位是step(通常是指epoch)。
        step_size, gamma = config['step_size'],config['gamma']
        scheduler = optim.lr_scheduler.StepLR(optimizer,step_size=step_size,gamma=gamma)
    elif sch_type == 'ReduceLROnPlateau':
        # 当某指标不再变化（上升或下降），调整学习率，非常实用的策略。例如，监测验证集的accuracy。
...
def main():
    ...
    ##
    to_use_device = torch.device(train_options['device'] if torch.cuda.is_available() and ('cuda' in train_optains['device']) else 'cpu')
    set_random_seed(cfg['SEED'], 'cuda' in train_option['device'], deterministic=True)
    ...
    if resume_from:
        ...
        global_state = {}
        logger.info('net resume from scratch.')
